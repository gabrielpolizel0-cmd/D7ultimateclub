...
// 🔐 Extende o tipo Player pra incluir is_admin
type PlayerWithAdmin = Player & { is_admin?: boolean };

export default function Header() {
  const router = useRouter();
  const [player, setPlayer] = useState<PlayerWithAdmin | null>(null);
                ...
                <div className="absolute right-0 mt-2 w-56 bg-gray-900 border border-gray-800 rounded-lg shadow-2xl overflow-hidden z-50">
                  ...
                  {player.is_admin && (
                    <Link
                      href="/painel"
                      onClick={() => setMenuOpen(false)}
                      className="block px-4 py-3 text-sm hover:bg-gray-800 transition-colors text-emerald-400 font-bold"
                    >
                      👑 Painel Admin
                    </Link>
                  )}
